...
class Bot:
    ...
    def fight(self,current_game_state,player):
        ...
        if player=="1":
            ...
            if self.exe_code == 0:  # Only record at the start of new action sequences
                if (  diff > 60 ) :
                    toss=np.random.randint(3)
                    if (toss==0):
                        action_sequence = [">","-","!>","v+>","-","!v+!>","v","-","!v","v+<","-","!v+!<","<+Y","-","!<+!Y"]
                    elif ( toss==1 ):
                        action_sequence = [">+^+B",">+^+B","!>+!^+!B"]
                    else:
                        action_sequence = ["<","-","!<","v+<","-","!v+!<","v","-","!v","v+>","-","!v+!>",">+Y","-","!>+!Y"]
                elif (  diff < -60 ) :
                    toss=np.random.randint(3)
                    ...
                    if ( toss>=1 ):
                        ...
                        action_sequence = ["v+R","v+R","v+R","!v+!R"]
                
                self.data_collector.record_action(current_game_state, action_sequence, self.exe_code)
                self.run_command(action_sequence, current_game_state.player1)
            else:
                self.run_command([], current_game_state.player1)
